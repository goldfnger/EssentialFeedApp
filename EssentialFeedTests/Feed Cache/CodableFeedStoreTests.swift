...
//  EssentialFeedTests
//
//  Created by Aleksandr Kornjushko on 24.10.2022.
//
...
final class CodableFeedStoreTests: XCTestCase {
  ...
  func test_insert_deliversErrorOnInsertionError() {
    let invalidStoreUL = URL(string: "invalid://store-url")
    let sut = makeSUT(storeURL: invalidStoreUL)
    let feed = uniqueImageFeed().local
    let timestamp = Date()
  ...
  private func expect( _ sut: FeedStore, toRetrieve expectedResult: RetrieveCachedFeedResult, file: StaticString = #filePath, line: UInt = #line) {
    ...
    sut.retrieve { retrievedResult in
        ...
        default:
          XCTFail("Expected to retrieve \(expectedResult), got \(retrievedResult)", file: file, line: line)
        }
