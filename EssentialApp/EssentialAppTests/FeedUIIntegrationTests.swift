...
class FeedUIIntegrationTests: XCTestCase {
  ...
  func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
    ...
    XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator state change for first view once second image loading completes error")
    XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected no loading indicator state change for second view once second image loading completes error")
  }
  ...
  func test_feedImageView_preloadsImageURLWhenNearVisible() {
    ...
    sut.loadViewIfNeeded()
    loader.completeFeedLoading(with: [image0, image1])
    XCTAssertEqual(loader.loadedImageURLs, [], "Expected no image URL requests untill image is near visible")
  ...
  func test_feedImageView_doesNotLoadImageAgainUntillPreviousRequestCompletes() {
    let image = makeImage(url: URL(string: "http://url-0.com")!)
    let (sut, loader) = makeSUT()
    sut.loadViewIfNeeded()
    // we load one image, but
    loader.completeFeedLoading(with: [image])
    
    // but simulate that it is near visible (start loading the image)
    sut.simulateFeedImageViewNearVisible(at: 0)
  ...
  // 'selection' will be used to 'notify handler' that something was 'done' eg 'selected'
  private func makeSUT(
    // selection will send the selected feed images
    ...
    let sut = FeedUIComposer.feedComposedWith(
      ...
      // we pass the selection to the feed composer
      selection: selection
    )
